...
def install_symlink(src, dst, install_dir, dst_is_dir=False, dir_mode=0o777):
    ...
    if new_dst.is_symlink() and os.readlink(new_dst) == src:
        print(f"File exists: {new_dst!r} -> {src!r}")
        return
    print(f"Installing symlink {new_dst!r} -> {src!r}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        usage=f"{pathlib.Path(__file__).name} [-h] [-d] [-m MODE] source dest install_dir\n\n"
        "example:\n"
        f"        {pathlib.Path(__file__).name} dash sh /bin",
    )
    parser.add_argument("source", help="target to link")
    parser.add_argument("dest", help="link name")
    parser.add_argument("install_dir", help="installation directory")
    parser.add_argument("-d", "--isdir", action="store_true", help="dest is a directory")
    parser.add_argument(
        "-m", "--mode", help="directory mode on creating if not exist", default="0o755"
    )
