...
def valgrindTest(valgrind, datagen, fuzzer, zstd, fullbench):
    ...
    if subprocess.call([*VALGRIND_ARGS, zstd], stdout=subprocess.DEVNULL) == 0:
        raise subprocess.SubprocessError("zstd without argument should have failed")

    with (
        subprocess.Popen([datagen, "-g80"], stdout=subprocess.PIPE) as p1,
        subprocess.Popen(
            [*VALGRIND_ARGS, zstd, "-", "-c"], stdin=p1.stdout, stdout=subprocess.DEVNULL
        ) as p2,
    ):
    ...
    with (
        subprocess.Popen([datagen, "-g16KB"], stdout=subprocess.PIPE) as p1,
        subprocess.Popen(
            [*VALGRIND_ARGS, zstd, "-vf", "-", "-c"], stdin=p1.stdout, stdout=subprocess.DEVNULL
        ) as p2,
    ):
        p1.stdout.close()
        p2.communicate()
    ...
    with tempfile.NamedTemporaryFile() as tmp_fd:
        with (
            subprocess.Popen([datagen, "-g2930KB"], stdout=subprocess.PIPE) as p1,
            subprocess.Popen(
                [*VALGRIND_ARGS, zstd, "-5", "-vf", "-", "-o", tmp_fd.name], stdin=p1.stdout
            ) as p2,
        ):
            p1.stdout.close()
            p2.communicate()
        ...
        subprocess.check_call(
            [*VALGRIND_ARGS, zstd, "-vdf", tmp_fd.name, "-c"], stdout=subprocess.DEVNULL
        )

        with (
            subprocess.Popen([datagen, "-g64MB"], stdout=subprocess.PIPE) as p1,
            subprocess.Popen(
                [*VALGRIND_ARGS, zstd, "-vf", "-", "-c"], stdin=p1.stdout, stdout=subprocess.DEVNULL
            ) as p2,
        ):
