...
def test_match_include() -> bool:
    if (
        include_regex.match('#include "file"')
        and include_regex.match('  #include "file"')
        and include_regex.match('#  include "file"')
        and include_regex.match('#include   "file"')
        and include_regex.match('#include "file" // comment')
    ):
        if (
            not include_regex.match("#include <file>")
            and not include_regex.match('//#include "file"')
            and not include_regex.match('/*#include "file"*/')
        ):
...
def test_match_pragma() -> bool:
    if (
        pragma_regex.match("#pragma once")
        and pragma_regex.match("  #pragma once")
        and pragma_regex.match("#  pragma once")
        and pragma_regex.match("#pragma   once")
        and pragma_regex.match("#pragma once // comment")
    ):
        if not pragma_regex.match("//#pragma once") and not pragma_regex.match("/*#pragma once*/"):
...
#
def resolve_include(file: str, parent: Path | None = None) -> Path | None:
    for root in roots:
...
def resolve_excluded_files(
    file_list: list[str] | None, resolved: set[Path], parent: Path | None = None
) -> None:
    if file_list:
                ...
                if match_include:
                    ...
                    if resolved:
                        if resolved in excludes:
                            ...
                            write_line(
                                f"#error Using excluded file: {inc_name} (re-amalgamate source to fix)"
                            )
...
parser = argparse.ArgumentParser(
    description="Amalgamate Tool",
    epilog=f"example: {sys.argv[0]} -r ../my/path -r ../other/path -o out.c in.c",
)
...
parser.add_argument(
    ...
    "--keep",
    action="append",
    help="file to exclude from inlining but keep the include directive",
)
parser.add_argument(
    "-p",
    "--pragma",
...
parser.add_argument(
    "-o", "--output", type=argparse.FileType("w"), help="output file (otherwise stdout)"
)
