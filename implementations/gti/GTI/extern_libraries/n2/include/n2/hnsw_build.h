...
namespace n2
{
    ...
    public:
        ...
        void AddDataM(const std::vector<float> &data);
        ...
        void saveNodes(const std::string &file_name); // Save nodes
        void loadNodes(const std::string &file_name); // Load nodes
        ...
        void insert(const std::vector<float> &data); // Insert data into Hnsw graph
        ...
        virtual void Link(HnswNode *source, HnswNode *target, int level, float dis) = 0;
        ...
        std::vector<const Data *> dptr_list_;
        std::vector<HnswNode *> nodes_;
        std::vector<HnswNode *> nodes_backup_;
        int num_nodes_ = 0;
        size_t data_dim_ = 0;
    ...
    protected:
        ...
        void Link(HnswNode *source, HnswNode *target, int level, float dis) override;
