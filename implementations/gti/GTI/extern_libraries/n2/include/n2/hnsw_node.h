...
namespace n2
{
    ...
    public:
        ...
        inline std::vector<float> &GetFriendsDis(int level) { return friends_dis_[level]; }
        ...
        size_t max_m_;
        size_t max_m0_;
        std::vector<unsigned> in_degrees; // In degree of each level;
        float in_degree_radius = 0;       // In degree radius;

        std::vector<std::vector<HnswNode *>> friends_at_layer_;
        std::vector<std::vector<float>> friends_dis_;
