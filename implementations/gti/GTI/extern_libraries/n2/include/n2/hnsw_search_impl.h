...
namespace n2
{
    ...
    public:
        ...
        void SearchById(int id, size_t k, int ef_search, bool ensure_k,
                        std::vector<std::pair<int, float>> &result) override;
        void SearchByVectorM(const std::vector<float> &qvec, size_t k, int ef_search, bool ensure_k,
                             std::vector<std::pair<int, float>> &result, std::vector<Neighbor> &result_leaf, std::vector<GTI_Entry *> &entries_sec, std::vector<std::vector<float>> &data) override;
        ...
        inline void CallSearchById_(int cur_node_id, float cur_dist, const float *qraw, size_t k, size_t ef_search,
                                    bool ensure_k, std::vector<std::pair<int, float>> &result, std::vector<Neighbor> &result_leaf,
                                    std::vector<GTI_Entry *> &entries_sec, std::vector<std::vector<float>> &data)
        {
            if (ensure_k)
            {
                while (result.size() < k && !ensure_k_path_.empty())
                {
                    ...
                    cur_dist = ensure_k_path_.back().second;
                    ensure_k_path_.pop_back();
                    SearchById_(cur_node_id, cur_dist, qraw, k, ef_search, ensure_k, result, result_leaf, entries_sec, data);
                }
            }
            else
            {
        ...
        template <typename ResultType>
        inline void SearchById_(int cur_node_id, float cur_dist, const float *qraw, size_t k, size_t ef_search,
                                bool ensure_k, ResultType &result, std::vector<Neighbor> &result_leaf, std::vector<GTI_Entry *> &entries_sec, std::vector<std::vector<float>> &data)
        {
            if (ef_search < k)
                SearchByIdV1_(cur_node_id, cur_dist, qraw, k, ef_search, ensure_k, result);
            else
                SearchByIdV2_(cur_node_id, cur_dist, qraw, k, ef_search, ensure_k, result, result_leaf, entries_sec, data);
        }
