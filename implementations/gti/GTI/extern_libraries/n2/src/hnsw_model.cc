...
namespace n2
{
        ...
        : enterpoint_id_(enterpoint_id), max_level_(max_level), data_dim_(data_dim), metric_(metric)
    {
        ...
        if (model_ == nullptr)
            throw runtime_error("[Error] Fail to allocate memory for optimised index (size: " + to_string(model_byte_size_ / (1024 * 1024)) + " MBytes)");
    ...
    void HnswModel::LoadConfigFromModel()
    {
        ...
        if (data_dim_bak > 0 && data_dim_ != data_dim_bak)
        {
            ...
            throw runtime_error("[Error] index dimension(" + to_string(data_dim_bak) + ") != model dimension(" + to_string(data_dim_) + ")");
        }
