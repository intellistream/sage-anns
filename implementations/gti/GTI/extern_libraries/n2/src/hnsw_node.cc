...
namespace n2
{
    ...
    {
        // in_degrees.assign(level + 1, 0);
        // in_degree_radius.assign(level + 1, 0.0);
        for (int i = 1; i <= level; ++i)
        {
            friends_at_layer_[i].reserve(max_m_ + 1);
            // friends_dis_[i].reserve(max_m_ + 1);
        }
        friends_at_layer_[0].reserve(max_m0_ + 1);
        // friends_dis_[0].reserve(max_m0_ + 1);
    }
    ...
    void HnswNode::Reset()
    {
        friends_at_layer_.resize(level_ + 1);
        for (int i = 1; i <= level_; ++i)
        {
            friends_at_layer_[i].clear();
            friends_at_layer_[i].reserve(max_m_ + 1);
            // friends_dis_[i].reserve(max_m_ + 1);
        }
        friends_at_layer_[0].clear();
        friends_at_layer_[0].reserve(max_m0_ + 1);
    }
