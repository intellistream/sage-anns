...
namespace n2
{
                                                       ...
                                                       bool ensure_k, ResultType &result)
    {
        ...
        for (auto i = model_->GetMaxLevel(); i > 0; --i)
        {
            ...
            while (changed)
            {
                ...
                const int *friends_with_size = (const int *)(model_higher_level_ + (offset + i - 1) * memory_per_node_higher_level_);
                ...
                for (auto j = 1; j <= size; ++j)
                {
                    ...
                    {
                        _mm_prefetch(qraw, _MM_HINT_T0);
                        const float *vec = (const float *)(model_level0_node_base_offset_ + fid * memory_per_node_level0_);
                        _mm_prefetch(vec, _MM_HINT_NTA);
                                                       ...
                                                       std::vector<std::vector<float>> &data)
    {
        ...
        // printf("oid: %d, radius: %f, size: %d\n", entries_sec[2]->oid, entries_sec[2]->radius, entries_sec[2]->child->entries.size());
        CallSearchById_(cur_node_id, cur_dist, qraw, k, ef_search, ensure_k, result, result_leaf, entries_sec, data);
    }
                                                     ...
                                                     size_t ef_search, bool ensure_k, ResultType &result)
    {
        ...
        while (!candidates.empty() and visited_cnt < ef_search)
        {
            ...
            const int *friends_with_size = (const int *)(model_level0_ + cur_node_id * memory_per_node_level0_ + sizeof(int));
            _mm_prefetch(friends_with_size, _MM_HINT_T0);
            int size = friends_with_size[0];
            ...
            for (auto j = 1; j <= size; ++j)
            {
                ...
                {
                    _mm_prefetch(qraw, _MM_HINT_T0);
                    const float *vec = (const float *)(model_level0_node_base_offset_ + node_id * memory_per_node_level0_);
                    _mm_prefetch(vec, _MM_HINT_NTA);
    ...
    template <typename ResultType>
    void HnswSearchImpl<DistFuncType>::SearchByIdV2_(int cur_node_id, float cur_dist, const float *qraw, size_t k,
                                                     size_t ef_search, bool ensure_k, ResultType &result, std::vector<Neighbor> &result_leaf, std::vector<GTI_Entry *> &entries_sec,
                                                     std::vector<std::vector<float>> &data)
    {
        IdDistancePairMinHeap candidates;
        IdDistancePairMinHeap visited_nodes;
        ...
        result_leaf.resize(ef_search + 1);
        for (unsigned i = 0; i < ef_search + 1; i++)
        {
            result_leaf[i] = (Neighbor(-1, 9999999, -1, true, -1));
        }
            ...
            for (auto j = 1; j <= size; ++j)
            {
                ...
                if (visited[node_id] != visited_mark)
                {
                    ...
                    if (d < found_distances.top() || found_distances.size() < ef_search)
                    {
                        ...
                        InsertIntoPool(result_leaf.data(), ef_search, Neighbor(entries_sec[node_id]->oid, d, node_id, true, 0));
                        ...
                        for (unsigned m = 1; m < child_entries.size(); m++)
                        {
                            ...
                            if (dis_leaf >= result_leaf[ef_search - 1].dis)
                                continue;
                            InsertIntoPool(result_leaf.data(), ef_search, Neighbor(oid, dis_leaf, node_id, true, m));
                        }
                    }
                }
            }
