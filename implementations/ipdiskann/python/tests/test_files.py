...
class TestVectorsFromFile(unittest.TestCase):
    def test_in_mem(self):
        ...
        with vectors_as_temp_file(expected) as vecs_file:
            ...
            self.assertTrue(
                (expected == actual).all(), f"{expected == actual}\n{expected}\n{actual}"
            )
    ...
    def test_memmap(self):
        ...
        with vectors_as_temp_file(expected) as vecs_file:
            ...
            actual = dap.vectors_from_file(vecs_file, dtype=np.float32, use_memmap=True)
            self.assertTrue(
                (expected == actual).all(), f"{expected == actual}\n{expected}\n{actual}"
            )
            ...
            actual = dap.vectors_from_file(
                vecs_file_copy.name, dtype=np.float32, use_memmap=True, mode="r+"
            )
            self.assertTrue(
                (expected == actual).all(), f"{expected == actual}\n{expected}\n{actual}"
            )
