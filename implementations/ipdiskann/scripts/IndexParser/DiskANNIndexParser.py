...
def main(index_path_prefix, data_type, output_file_prefix, use_pq_vectors):
    ...
    print(
        str.format(
            "Parsing DiskANN index at {0} with data type {1} and writing output to {2}. Use PQ vectors: {3}",
            index_path_prefix,
            data_type,
            output_file_prefix,
            use_pq_vectors,
    ...
    if use_pq_vectors:
        pdi.parse_index_with_PQ_vectors(
            index_path_prefix,
            data_type_code,
            data_type_size,
            out_disk_index_file,
    ...
    print(
        "Parsed DiskANN index and wrote output to "
        + out_disk_index_file
        + ", "
        + out_pq_vectors_file
        + ", "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "index_path_prefix", type=str, help="Path to the DiskANN index file without the extension"
    )
    parser.add_argument(
        "data_type",
        type=str,
        help="Data type of the vectors in the index. Supported data types are float, int8 and uint8",
    )
    parser.add_argument(
        "output_file_prefix",
        type=str,
        help="Output file prefix to write index and PQ vectors. The index is written in CSV format with the following columns: Id, vector, neighbours",
    )
    parser.add_argument(
        "--use_pq_vectors",
        default=False,
        action="store_true",
        help="Whether to replace FP vectors with PQ vectors in the output file.",
    )
