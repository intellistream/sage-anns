...
def parse_index(index_path_prefix, data_type_code, data_type_size, out_graph_csv):
    return parse_index_with_PQ_vectors(
        index_path_prefix, data_type_code, data_type_size, out_graph_csv, None
    )


def parse_index_with_PQ_vectors(
    index_path_prefix, data_type_code, data_type_size, out_graph_csv, compressed_vectors=None
):
    ...
    with open(out_graph_csv, "w") as out_file:
        ...
        with open(disk_index_file_name, "rb") as index_file:
            ...
            if num_dims != 1 or num_entries != 9:
                raise Exception(
                    "Mismatch in metadata. Expected 1 dimension and 9 entries. Got "
                    + str(num_dims)
                    + " dimensions and "
                    + str(num_entries)
                    + " entries."
            ...
            while nodes_read < num_nodes:
                ...
                if sectors_per_node == 1:
                    ...
                    nodes = process_multi_sector_node(
                        index_file, data_type_code, num_dims, nodes_read
                    )
                    assert len(nodes) == 1
    ...
    print(
        "Processed "
        + str(nodes_read)
        + " points from index in "
        + disk_index_file_name
        + " and wrote output to "
        + out_graph_csv
    )
