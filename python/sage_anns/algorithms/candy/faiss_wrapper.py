...
class FAISSIndex(BaseANNSIndex):
    ...
    def __init__(self, dimension: int, metric: str = "l2", index_type: str = "Flat", **kwargs):
...
class FAISSHNSWIndex(BaseANNSIndex):
    ...
    def build(self, data: np.ndarray) -> None:
        ...
        self._index = PyCANDY.IndexHNSWFlatOptimized(int(self.dimension), int(self.M), metric_type)
    ...
    def search(
        self, queries: np.ndarray, k: int = 10, ef: int | None = None, **search_params
    ) -> tuple[np.ndarray, np.ndarray]:
