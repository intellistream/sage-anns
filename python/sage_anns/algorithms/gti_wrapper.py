...
class GTIIndex(BaseANNSIndex):
    ...
    def build(self, data: np.ndarray) -> None:
        ...
        self._index.setup(
            self.max_pts, self.dimension, self.capacity_up_i, self.capacity_up_l, self.m
        )
        ...
        self._index.build(data, ids, self.capacity_up_i, self.capacity_up_l, self.m)
    ...
    def add(self, vectors: np.ndarray, ids: np.ndarray | None = None) -> None:
        ...
        if ids is None:
            ids = np.arange(self._next_id, self._next_id + n_samples, dtype=np.int32)
            self._next_id += n_samples
        else:
    ...
    def search(
        ...
        **search_params,
        ...
        search_L = L if L is not None else self.L  # noqa: N806
