...
These interfaces are used by benchmark and service-layer packages.
Concrete implementations are provided by the ``isage-anns`` package.
"""
...
class AnnIndex(ABC):
    ...
    def batch_search(
        self, queries: np.ndarray, k: int, *, timestamps: np.ndarray | None = None
