...
@pytest.mark.skipif(not _is_pycandy_available(), reason="PyCANDYAlgo not built")
def test_candy_flat_basic():
    """Test CANDY Flat basic functionality."""
    from sage_anns import create_index
...
def test_faiss_hnsw_basic():
    ...
    index = create_index("faiss_hnsw", dimension=dimension, metric="l2", M=8)
...
@pytest.mark.skipif(not _is_vsag_available(), reason="pyvsag not installed")
def test_vsag_hnsw_basic():
    ...
    index = create_index("vsag_hnsw", dimension=dimension, metric="l2", M=8)
...
@pytest.mark.skipif(not _is_gti_available(), reason="gti_wrapper not built")
def test_gti_basic():
...
@pytest.mark.skipif(not _is_plsh_available(), reason="plsh_python not built")
def test_plsh_basic():
    ...
    index = create_index("plsh", dimension=dimension, metric="l2", k=4, m=5)
